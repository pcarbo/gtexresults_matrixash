...
#   png(file, width = width, height = height, units = 'cm', res = 300)
# }
# 
# knitr::opts_chunk$set(dev = 'my_png', fig.ext = 'png')
```
...
```{r effectorderfoldsig,echo=F,fig.height=5,fig.width=5}
library('colorRamps')
library(gplots)
library(ggplot2)
brain.only.order=read.table("../../Dropbox/brain.only.order.txt")[,1]
...
heatmap.2(shared.fold.size[(all.tissue.order),all.tissue.order],Rowv=FALSE,Colv=FALSE,
...
mtext("Defined as QTL if LFSR<0.05")
heatmap.2(shared.fold.size.nobrain[(exclude.brain.order),exclude.brain.order],Rowv=FALSE,Colv=FALSE,
...
heatmap.2(shared.fold.size.brainonly[(brain.only.order),brain.only.order],Rowv=FALSE,Colv=FALSE,
        symm=TRUE,dendrogram="none",density="none",trace="none",col=blue2green,main=paste0("eQTL in either tissues and within 2-fold,SUB"),cexRow=0.5,cexCol=0.5,cex.main=0.5,breaks=seq(0.35,1,0.01))$rowInd
mtext("Defined as QTL if LFSR<0.05")

##try with upper triangular
